...
// Logic assumes CCW. Reflex => Interior angle > 180 => Right Turn (-Z)
bool Polygon::is_reflex(size_t i) const {
    const Point prev = get_vertex(i + vertices.size() - 1), curr = get_vertex(i), next = get_vertex(i + 1);
...
bool segments_intersect(const Segment &s1, const Segment &s2) {
    ...
    const int o1 = orientation(p1, q1, p2), o2 = orientation(p1, q1, q2), o3 = orientation(p2, q2, p1),
