...
bool Polygon::is_reflex(size_t i) const {
    const Point prev = get_vertex(i + vertices.size() - 1);
    const Point curr = get_vertex(i);
    const Point next = get_vertex(i + 1);
...
bool is_point_in_polygon(const Polygon& P, const Point& p) {
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        bool intersect_y = ((v1.y > p.y) != (v2.y > p.y));
        if (intersect_y) {
            double x_inters = (v2.x - v1.x) * (p.y - v1.y) / (v2.y - v1.y) + v1.x;
            if (p.x < x_inters) inside = !inside;
        }
        if (on_segment(p, {v1, v2})) return true; // Boundary inclusion
    }
    return inside;
}
...
bool is_visible_naive(const Polygon& P, Point q, Point r) {
    ...
    // IMPORTANT FIX: Strict Interior Check
    // A segment connected by two points must actully BE inside the polygon.
